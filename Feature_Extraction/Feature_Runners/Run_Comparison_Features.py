...
def run_all_comparison_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    paragraph_length_std_columns = ["a_paragraph_length_std", "b_paragraph_length_std"]

    if all(col in df.columns for col in paragraph_length_std_columns):
        comparison_features = df.apply(
    ...
    has_next_steps_columns = ["a_has_next_steps", "b_has_next_steps"]

    if all(col in df.columns for col in has_next_steps_columns):
        comparison_features = df.apply(
    ...
    long_sentence_count_columns = ["a_long_sentence_count", "b_long_sentence_count"]

    if all(col in df.columns for col in long_sentence_count_columns):
        comparison_features = df.apply(
